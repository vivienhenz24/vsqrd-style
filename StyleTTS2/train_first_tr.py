...
def load_alignments(paths, alignment_dir, input_lengths, mel_input_length, n_down, device):
    ...
    Returns: attn (B, T_text, T_frames) on device, already monotonic (hard).
    """
    B = len(paths)
...
def main(config_path):
    ...
    del model.text_aligner  # not used — alignments are pre-computed
    ...
    n_down = 1  # ASRCNN stride-2 downsample factor (hardcoded, no aligner loaded)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            waves = batch[0]
            paths = batch[-1]  # list of wav paths, not moved to device
